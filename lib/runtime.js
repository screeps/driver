...
var getSandboxedEval = new vm.Script("(eval)");
...
function checkOriginalPrototypes() {
    ...
    if(Object.getOwnPropertyNames(Array.prototype).some(prop => {
            return prop != '__proto__' && prop != 'length' && !originalPrototypes.Array.protoMethods.hasOwnProperty(prop);
        })) {
...
process.on('message', (message) => {
    ...
    .then((data) => {
        ...
        try {
            ...
            var sandboxedEval = getSandboxedEval.runInContext(globals);
            ...
            if(segmentKeys.length > 0) {
                if(segmentKeys.length > 10) {
                    return q.reject('Cannot save more than 10 memory segments on the same tick');
                }
