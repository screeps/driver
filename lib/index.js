...
Object.defineProperty(config.engine, 'registerCustomObjectPrototype', {
    value: function(objectType, name, opts) {
        ...
        for(const prop in opts.properties) {
          opts.properties[prop] = '' + opts.properties[prop]
        }
        if(opts.prototypeExtender) {
          opts.prototypeExtender = '' + opts.prototypeExtender
        }
...
exports.connect = function(processType) {
    ...
    .then(() => {
        ...
        if (processType == 'main') {
            ...
            env.get(env.keys.MAIN_LOOP_MIN_DURATION)
                .then(setTickRate);
...
exports.getAllRoomsNames = async function getAllRoomsNames() {
    const result = await env.smembers(env.keys.ACTIVE_ROOMS);
    await env.del(env.keys.ACTIVE_ROOMS);
    return result;
};

const activateRoom = exports.activateRoom = function activateRoom(room) {
...
exports.saveUserIntents = function(userId, intents) {
    ...
    if(activeRooms.size > 0) {
        updates.push(activateRoom(Array.from(activeRooms)));
    }
...
exports.sendConsoleMessages = function(userId, messages) {
    ...
    messages.log = messages.log.map(item => item.escape ? he.encode(item.message) : item.message);
...
exports.updateRoomStatusData = async () => {
    const now = Date.now();
    const rooms = await db.rooms.find({$or: [
            {novice: {$gt: now}},
            {respawnArea: {$gt: now}},
            {openTime: {$gt: now}},
    ...
    return env.set(env.keys.ROOM_STATUS_DATA, JSON.stringify(statusData));
};
...
exports.system = common.configManager.config.common.system;
