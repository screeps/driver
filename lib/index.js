var bulk = require('./bulk'),
    ...
    config = Object.assign(common.configManager.config, {engine: {}}),
    q = require('q'),
    vm = require('vm'),
    _ = require('lodash'),
...
_.extend(config.engine, {
    ...
    onMainLoopStage(stage) {},
    mainLoopCustomStage() {
        return q.when();
    },
    onRunnerLoopStage(stage, userId) {},
    onProcessorLoopStage(stage, roomId) {},
    cpuMaxPerTick: 500,
    cpuBucketSize: 10000,
    onPlayerSandbox(sandbox, userId) {},
    onProcessObject(object, roomObjects, roomTerrain, gameTime, roomInfo, objectsUpdate, usersUpdate) {
        return true;
    },
    onProcessObjectIntents(object, userId, intents, roomObjects, roomTerrain, gameTime, roomInfo, objectsUpdate, usersUpdate) {},
    onProcessRoom(roomId, roomInfo) {},
    onSaveRoomHistory(roomId, baseTime, data) {},
    customIntentTypes: {}
});
...
exports.connect = function(processType) {
    ...
    .then(() => {
        ...
        return config.engine.onInit(processType);
    });
};
...
exports.getRuntimeData = function(userId, onlyInRoom) {
    ...
    .then((_userObjects) => {
        ...
        var userIdsHash = {[userId]: true}, userIds = [];
        result[6].forEach((i) => {
            if(i.user) {
                        ...
                        if (runtimeData.cpu < Infinity) {
                            runtimeTimeout = setTimeout(() => {
                                ...
                                runtimeDeferred.reject({
                                    type: 'error',
                                    error: 'Script execution has been terminated: CPU Limit reached'
                                });
                                ...
                                db.users.update({_id: runtimeData.user._id}, {$inc: {cpuAvailable: runtimeData.cpu-600}});
