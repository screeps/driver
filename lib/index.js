...
global._nativeMod = require('../native/build/Release/native');
...
exports.getAllUsers = function() {
    ...
    .then((data) => {
        ...
        if(data[1].length) {
            data[1] = _.pluck(data[1], 'room');
            data[1] = _.uniq(data[1]);
            data[1].forEach((i) => data[0].push({_id: 'Invader:' + i}));
        }
        return {ivm: data[0], legacy: []};
    })
};
