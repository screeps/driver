...
exports.markPendingHistory = function(roomId, baseTime) {
    var key = env.keys.ROOM_HISTORY + 'pending:' + baseTime;
    return env.sadd(key, roomId)
        .then(() => env.expire(key, 3600));
};

exports.takePendingHistory = function(baseTime) {
    var key = env.keys.ROOM_HISTORY + 'pending:' + baseTime;
    return env.smembers(key)
        .then(rooms => env.del(key).then(() => rooms || []));
};
