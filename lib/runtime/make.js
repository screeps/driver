...
function getAllTerrainData() {
    ...
    return runtimeData.getAllTerrainData()
        .then((result) => {
            ...
            console.log('Static terrain data size:', staticTerrainDataSize);
        })
}
...
async function make (scope, userId, onlyInRoom) {
    ...
    try {
        ...
        config.engine.emit('playerSandbox', {
            ...
            set(name, value) {
                return vm.jail.setSync(name, new ivm.ExternalCopy(value).copyInto());
            },
            get(name) {
                return vm.jail.getSync(name).copySync();
            },
            getIsolate() {
                return vm.isolate;
            },
